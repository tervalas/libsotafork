...
-- Latest revision depends on libsota 0.5.x by tervalas
...
function createLabel(caption, font, left, top, width, height, parentID, parentKind)
	--local r = rect(left, top, width, height)
	return ui.label.add(caption, font, left, top, width, height, parentID, parentKind)
end

function createLabelWithShadow(caption, font, left, top, width, height, parentID, parentKind)
	local s = ui.label.add(caption, font, left, top, width, height, parentID, parentKind)
	local l = ui.label.add(caption, font, left-1, top-1, width, height, s.objId, s.objKind)		
	l.shadow = s
	return l
end
...
function setLabelCaption(label, caption)
	...
	if label.shadow then
		...
		label.shadow:setCaption("<color=black>"..c.."</color>")
	end	
end
...
function createTexture(filename)
	--local r = rect(left, top, width, height)
	return ui.texture.add(filename)
end
...
function createImage(left, top, width, height, texture, parentID, parentKind)
	return ui.image.add(left, top, width, height, texture, parentID, parentKind)
end
